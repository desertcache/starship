...
```
src/core/    — engine bootstrap, render loop, perf telemetry, named-camera registry
src/world/   — ship modules, ONE FILE PER ROOM (cockpit.ts, corridor.ts, quarters.ts, galley.ts, engineering.ts) + assembly.ts that positions them
...
2. `npm run verify` — then **read every screenshot in verify/shots/** with your own eyes. Compare against the phase acceptance criteria and the Visual Direction below. If a room looks wrong, broken, unlit, or ugly: fix it. Do not rationalize a bad screenshot.
3. Read `verify/report.json`. Budget (from headed run): avgFps ≥ 60, p95 frame time ≤ 18ms, draw calls ≤ 300, triangles ≤ 500k.
...
- Lighting: one HemisphereLight + ≤6 PointLights total across the whole ship. Everything else reads as lit via emissive materials. Shadows off by default; if enabled, one light max.
