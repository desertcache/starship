...
import { getFlowW, getFlowAxis } from '../../flight/flightShim.js'; // LANE-C SHIM — replaced by flightState at merge
...
export interface BodyEntry {
   ...
   *  Same magnitudes as the old (vx, vy, driftSpeed) triple. */
...
export function spawnHero(rng: Rng, o: HeroOpts = {}): BodyEntry {
  ...
  return {
    ...
    driftW: new THREE.Vector3(
      o.vx ?? rng.signed(1.4),
      o.vy ?? rng.signed(1.0),
      o.driftSpeed ?? heroDriftFor(r, rng),
    ),
    radius: r,
  };
}
...
export function spawnAmbient(rng: Rng): BodyEntry {
  ...
  return {
    ...
    role: 'AMBIENT',
    body,
    driftW: new THREE.Vector3(rng.signed(2.0), rng.signed(1.6), rng.range(14, 24)),
    radius: r,
  };
}
